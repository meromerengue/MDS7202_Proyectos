...
def optim(model, X_train, X_test, y_train, y_test):
    ...
    def objective(trial):
        #hyperparameters = {
        if tipo_modelo == type(DummyClassifier()).__name__:
        #"Dummy": 
            grilla = {
            "strategy": trial.suggest_categorical("strategy", ["stratified", "most_frequent", "prior", "uniform", "constant"]),
            }#,
        elif tipo_modelo == type(LogisticRegression()).__name__:
        #"LogisticRegression": 
            grilla = {
            "C": trial.suggest_loguniform("C", 1e-6, 1e6),
            "solver": trial.suggest_categorical("solver", ["newton-cg", "lbfgs", "liblinear", "sag", "saga"]),
            }#,
        elif tipo_modelo == type(DecisionTreeClassifier()).__name__:
        ...
        elif tipo_modelo == type(SVC()).__name__:    
        #"SVC": 
            grilla = {
            "C": trial.suggest_loguniform("C", 1e-6, 1e6),
        ...
        elif tipo_modelo == type(RandomForestClassifier()).__name__:  
        ...
        elif tipo_modelo == type(LGBMClassifier()).__name__:      
        #"LightGBMClassifier": 
            grilla = {
        ...
        elif tipo_modelo == type(XGBClassifier()).__name__:        
        #"XGBClassifier": 
            grilla = {
            "learning_rate": trial.suggest_loguniform("learning_rate", 0.01, 0.1),
            "max_depth": trial.suggest_int("max_depth", 1, 32),
